...
public class MainActivity extends AppCompatActivity {
    ...
    private String ruta_captura_video;
    private static final String SUFIJO_VIDEO = ".mp4";
    private static final String PREFIJO_VIDEO = "ALEX_VIDEO_";
    ...
    private Uri crearFicheroVideo()
    {
        ...
        File f = null;
            ...
            ruta_captura_video = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_MOVIES).getPath()+"/"+nombre_fichero;

            Log.d(getClass().getCanonicalName(), "RUTA FOTO = " + ruta_captura_video);
    ...
    private void hacerVideo()
    {

        Intent intent = new Intent(MediaStore.ACTION_VIDEO_CAPTURE); //PARA MANDAR EL INTENT DE VIDEO

        Uri video_uri = crearFicheroVideo();
        intent.putExtra(MediaStore.EXTRA_OUTPUT, video_uri); //He aquí la parte opcional del código: de emplearse este parámetro, la foto tomada se almacena en una localización concreta y de omitirse, se alamcena en una localización aleatoria
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if ((tienePermisos (android.Manifest.permission.CAMERA))&&(tienePermisos (android.Manifest.permission.WRITE_EXTERNAL_STORAGE)))
        {
            hacerVideo();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (resultCode)
        {
            case RESULT_OK:
                ...
                MediaController mediaController = new MediaController(this);//Añadir los controles al video
                mediaController.setAnchorView(videoView); //ancho de los controles
                ...
                videoView.start();
                   ...
                   /* Bitmap bitmap = null; //la foto que se mostrará en la actividad
                    ...
                    if (data == null)//el fichero ha sido guarado en una ruta => se ha usado el putExtra MediaStore.EXTRA_OUTPUT
                    {
                        ...
                        try
                        {
                            File imgFile = new  File(ruta_captura_video);
